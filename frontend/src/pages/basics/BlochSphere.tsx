    ...
    <div className="flex flex-col h-full bg-transparent text-black-olive">
      ...
      <div className="space-y-4 text-sm sm:text-base leading-relaxed text-black-olive/80 mb-6 flex-grow">
        <ul className="list-disc pl-5 space-y-2 text-olive-mist">
          <li><span className="text-black-olive/90">The <strong>Bloch sphere</strong> is a geometric representation of any single pure qubit state on the surface of a 3D unit sphere.</span></li>
          <li><span className="text-black-olive/90">Any pure state maps to <span className="font-mono text-black-olive font-semibold">|ψ⟩ = cos(θ/2)|0⟩ + e^(iφ)sin(θ/2)|1⟩</span>.</span></li>
          <li><span className="text-black-olive/90">The North pole corresponds to <span className="font-mono font-bold text-black-olive">|0⟩</span>, and the South pole to <span className="font-mono font-bold text-black-olive">|1⟩</span>.</span></li>
          <li><span className="text-black-olive/90">The equator represents equal superposition states with varying relative phase: |+⟩, |−⟩, |i⟩, and |−i⟩.</span></li>
          <li><span className="text-black-olive/90">Quantum gates physically act as unitary rotations of this state vector about the sphere's axes.</span></li>
        </ul>
      </div>

      {/* Botanical Inset Panel */}
      <div className="flex flex-col md:flex-row gap-8 items-center bg-warm-ivory/60 border border-soft-sand p-6 rounded-2xl shadow-sm mb-8">
        <div className="flex-1 w-full flex justify-center">
          <svg viewBox="0 0 400 400" className="w-full max-w-sm font-sans select-none" fill="none">
            ...
            <line 
              ...
              stroke="#203C3D" 
              strokeWidth="3.5" 
              markerEnd="url(#blochNeuArrow)" 
            />

            {/* State vector point */}
            <circle cx={cx + x2d} cy={cy + y2d} r="5" fill="#203C3D" />
            <text 
              x={cx + x2d + 12} 
              y={cy + y2d - 6} 
              fill="#203C3D" 
        ...
        <div className="flex-1 flex flex-col gap-5 w-full">
          <div>
            <div className="flex justify-between text-xs text-black-olive mb-2 font-medium">
              <span>Polar Angle (θ)</span>
              <span className="text-black-olive font-mono font-bold">{(theta / Math.PI).toFixed(2)}π</span>
            </div>
            <input 
              type="range" 
              min="0" 
            ...
            />
            <div className="flex justify-between text-[10px] text-olive-mist font-mono mt-1">
              <span>0 (North: |0⟩)</span>
              <span>π (South: |1⟩)</span>
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between text-xs text-black-olive mb-2 font-medium">
              ...
              <span className="text-black-olive font-mono font-bold">{(phi / Math.PI).toFixed(2)}π</span>
            </div>
            <input 
              type="range" 
              min="0" 
          ...
          <div className="bg-warm-ivory border border-soft-sand p-4 rounded-xl mt-2 space-y-1 shadow-sm">
            <div className="text-[10px] text-olive-mist uppercase tracking-wider font-semibold">Current State Equation</div>
            <div className="font-mono text-sm md:text-base text-black-olive font-bold">
